...
/* stdlib_ng.mtlx implements hsvadjust as RGB->HSV, hue offset, saturation/value
...
/* Exact zero-contribution subset only: graph.cpp admits
 * ND_generalized_schlick_bsdf when literal weight=0.0, because MaterialX's
 * implementation returns before contributing a closure in that case. */
...
bool read_typed_value_operand(const pxr::UsdShadeShader &shader,
                              const string &nodedef,
                              const char *input_name,
                              Type type,
                              Graph *graph,
                         ...
                         string *error_message)
{
  ...
  if (is_vector4_ramp(nodedef) || is_vector4_split(nodedef)) {
    ...
    for (const char *input_name : {first_name, second_name}) {
      if (!read_typed_value_operand(source_shader,
                                    nodedef,
                                    input_name,
                                    Type::Vector4,
                                    graph,
                          ...
                          string *error_message)
{
  ...
  if (nodedef == multiply_matrix33_id || nodedef == divide_matrix33_id) {
    ...
    for (const char *input_name : {"in1", "in2"}) {
      const pxr::UsdShadeInput matrix_input = source_shader.GetInput(pxr::TfToken(input_name));
      if (!matrix_input || matrix_input.GetTypeName() != pxr::SdfValueTypeNames->Matrix3d ||
          matrix_input.HasConnectedSource() ||
          !read_conditional_value_operand(source_shader,
                                          nodedef,
                                          ...
                                          error_message))
      {
        set_error(error_message, nodedef + " requires literal finite matrix33 inputs");
        return finish(false);
      }
    }
    if (source_shader.GetInputs().size() != 2 || source_shader.GetOutputs().size() != 1 ||
    ...
    *result = {multiply.name, "out", Type::Matrix33};
    emitted_shaders->emplace(shader_path, multiply.name);
    graph->nodes.push_back(std::move(multiply));
    return finish(true);
  }

  if (nodedef == transpose_matrix33_id) {
    Node transpose;
    transpose.name = unique_node_name(*graph, source_shader.GetPrim().GetName().GetString(), shader_path);
    transpose.nodedef = nodedef;
    const pxr::UsdShadeInput matrix_input = source_shader.GetInput(pxr::TfToken("in"));
    if (!matrix_input || matrix_input.GetTypeName() != pxr::SdfValueTypeNames->Matrix3d ||
        matrix_input.HasConnectedSource() ||
        !read_conditional_value_operand(source_shader,
    ...
    transpose.outputs["out"] = Type::Matrix33;
    *result = {transpose.name, "out", Type::Matrix33};
    emitted_shaders->emplace(shader_path, transpose.name);
    graph->nodes.push_back(std::move(transpose));
    return finish(true);
  }

  if (is_matrix33_literal_select(nodedef)) {
    Node conditional;
    conditional.name = unique_node_name(
        *graph, source_shader.GetPrim().GetName().GetString(), shader_path);
    conditional.nodedef = nodedef;
    if (is_integer_predicate_conditional(nodedef)) {
      ...
      for (const char *name : {"value1", "value2"}) {
        const pxr::UsdShadeInput predicate = source_shader.GetInput(pxr::TfToken(name));
        bool value = false;
        if (!predicate || predicate.GetTypeName() != pxr::SdfValueTypeNames->Bool ||
            predicate.HasConnectedSource() || !predicate.Get(&value))
        {
          set_error(error_message, nodedef + " requires literal boolean predicate inputs");
          return finish(false);
        }
      ...
      for (const char *name : {"value1", "value2"}) {
        const pxr::UsdShadeInput predicate = source_shader.GetInput(pxr::TfToken(name));
        float value = 0.0f;
        if (!predicate || predicate.GetTypeName() != pxr::SdfValueTypeNames->Float ||
            predicate.HasConnectedSource() || !predicate.Get(&value) || !std::isfinite(value))
        {
          set_error(error_message, nodedef + " requires literal finite float predicate inputs");
          return finish(false);
        }
        conditional.inputs[name] = value;
      }
    }
    for (const char *input_name : {"in1", "in2"}) {
      const pxr::UsdShadeInput matrix_input = source_shader.GetInput(pxr::TfToken(input_name));
      if (!matrix_input || matrix_input.GetTypeName() != pxr::SdfValueTypeNames->Matrix3d ||
          ...
          !read_conditional_value_operand(source_shader,
                                          ...
                                          error_message))
      {
        set_error(error_message, nodedef + " requires literal finite matrix33 result arms");
        return finish(false);
      }
    }
    if (source_shader.GetInputs().size() != 4 || source_shader.GetOutputs().size() != 1 ||
        ...
        source_shader.GetOutput(pxr::TfToken("out")).GetTypeName() != pxr::SdfValueTypeNames->Matrix3d)
    {
      set_error(error_message, nodedef + " requires value1/value2/in1/in2 and matrix33 output 'out'");
      return finish(false);
    }
  ...
  set_error(error_message,
           "MaterialX Matrix33 node '" + nodedef +
               "' is not a supported native Matrix33 lowerer (constant, literal arithmetic, "
               "creatematrix, literal conditionals, and literal-selector switches are implemented)");
  return finish(false);
}
                          ...
                          string *error_message)
{
  ...
  if (nodedef == multiply_matrix44_id || nodedef == divide_matrix44_id) {
    ...
    multiply.nodedef = nodedef;
    for (const char *input_name : {"in1", "in2"}) {
      if (!read_matrix44_literal_operand(source_shader, nodedef, input_name, &multiply, error_message) ||
          !matrix44_literal_is_finite_affine(multiply.matrix44_inputs.at(input_name)))
      {
        set_error(error_message, nodedef + " requires literal affine matrix44 inputs");
        return finish(false);
      }
    }
    if (source_shader.GetInputs().size() != 2 || source_shader.GetOutputs().size() != 1 ||
    ...
    multiply.outputs["out"] = Type::Matrix44;
    *result = {multiply.name, "out", Type::Matrix44};
    emitted_shaders->emplace(shader_path, multiply.name);
    graph->nodes.push_back(std::move(multiply));
    return finish(true);
  }
  ...
  if (nodedef == transpose_matrix44_id) {
    Node transpose;
    transpose.name = unique_node_name(*graph, source_shader.GetPrim().GetName().GetString(), shader_path);
    transpose.nodedef = nodedef;
    if (!read_matrix44_literal_operand(source_shader, nodedef, "in", &transpose, error_message)) {
      return finish(false);
    }
    if (source_shader.GetInputs().size() != 1 || source_shader.GetOutputs().size() != 1 ||
        !source_shader.GetOutput(pxr::TfToken("out")) ||
    ...
    if (!matrix44_literal_is_finite_affine(matrix44_transpose_result(transpose))) {
      set_error(error_message, nodedef + " result is not an affine matrix44");
      return finish(false);
    }
    transpose.outputs["out"] = Type::Matrix44;
    *result = {transpose.name, "out", Type::Matrix44};
    emitted_shaders->emplace(shader_path, transpose.name);
    graph->nodes.push_back(std::move(transpose));
    return finish(true);
  }
  ...
  if (is_matrix44_literal_select(nodedef)) {
    Node conditional;
    conditional.name = unique_node_name(
        *graph, source_shader.GetPrim().GetName().GetString(), shader_path);
    conditional.nodedef = nodedef;
    ...
    for (const char *input_name : {"in1", "in2"}) {
      if (!read_matrix44_literal_operand(source_shader, nodedef, input_name, &conditional, error_message)) {
        return finish(false);
      }
    }
    if (source_shader.GetInputs().size() != 4 || source_shader.GetOutputs().size() != 1 ||
        ...
        source_shader.GetOutput(pxr::TfToken("out")).GetTypeName() != pxr::SdfValueTypeNames->Matrix4d)
    {
      set_error(error_message, nodedef + " requires value1/value2/in1/in2 and matrix44 output 'out'");
      return finish(false);
    }
  ...
  set_error(error_message,
           "MaterialX Matrix44 node '" + nodedef +
               "' is not a supported native Matrix44 lowerer (literal add/subtract, creatematrix, "
               "ND_constant_matrix44, literal conditionals, and literal-selector switches are implemented)");
  return finish(false);
}
                        ...
                        string *error_message)
{
  ...
  if (nodedef == hsvadjust_color4_id) {
    if (!shader_has_exact_signature(source_shader, {"in", "amount"}, {"out"}, error_message)) {
      return finish(false);
    }
    const pxr::UsdShadeOutput output = source_shader.GetOutput(pxr::TfToken("out"));
    ...
    pxr::GfVec3f amount;
    if (!amount_input || amount_input.GetTypeName() != pxr::SdfValueTypeNames->Float3 ||
        amount_input.HasConnectedSource() || !amount_input.Get(&amount) ||
        !std::isfinite(amount[0]) || !std::isfinite(amount[1]) || !std::isfinite(amount[2]))
    {
    ...
    Node adjust;
    adjust.name = unique_node_name(*graph, source_shader.GetPrim().GetName().GetString(), shader_path);
    adjust.nodedef = nodedef;
    if (input.HasConnectedSource()) {
      Link link;
      if (!read_color4_output(input, graph, &link, active_shaders, emitted_shaders, depth + 1, error_message)) {
        return finish(false);
      }
      adjust.links["in"] = link;
    }
    else {
      pxr::GfVec4f value;
      if (!input.Get(&value) || !color4_is_finite(value)) {
      ...
      adjust.float4_inputs["in"] = make_float4(value[0], value[1], value[2], value[3]);
    }
    adjust.vector3_inputs["amount"] = make_float3(amount[0], amount[1], amount[2]);
    adjust.outputs["out"] = Type::Color4;
    *result = {adjust.name, "out", Type::Color4};
    emitted_shaders->emplace(shader_path, adjust.name);
                       ...
                       std::unordered_map<string, string> *emitted_float_shaders)
{
  ...
  if (nodedef == grid_color3_id || nodedef == crosshatch_color3_id) {
    ...
    for (const char *input_name : {"uvtiling", "uvoffset"}) {
      ...
      grid.vector2_inputs[input_name] = make_float2(value[0], value[1]);
    }
  ...
  if (nodedef == hsvadjust_color3_id) {
    ...
    if (input.HasConnectedSource()) {
      ...
      adjust.color3_inputs["in"] = make_float3(value[0], value[1], value[2]);
    }
    adjust.vector3_inputs["amount"] = make_float3(amount[0], amount[1], amount[2]);
...
bool read_typed_value_operand(const pxr::UsdShadeShader &shader,
                              ...
                              string *error_message)
{
  return read_conditional_value_operand(shader,
                                        nodedef,
                                        input_name,
                                        type,
                                        graph,
                         ...
                         string *error_message)
{
  ...
  if ((is_vector2_ramp(nodedef) || is_vector2_split(nodedef))) {
    ...
    for (const char *input_name : {first_name, second_name}) {
      if (!read_typed_value_operand(source,
                                    nodedef,
                                    input_name,
                                    Type::Vector2,
                                    graph,
                       ...
                       string *error_message)
{
  ...
  else if (is_worleynoise_float(nodedef)) {
    const bool is_3d = nodedef == worleynoise3d_float_id;
    const char *coordinate_name = is_3d ? "position" : "texcoord";
    const pxr::SdfValueTypeName coordinate_type = is_3d ? pxr::SdfValueTypeNames->Float3 :
                                                         pxr::SdfValueTypeNames->Float2;
    if (!shader_has_exact_signature(source, {coordinate_name, "jitter", "style"}, {"out"}, error_message) ||
    ...
    for (const pxr::UsdShadeInput &closure_input : closure.GetInputs()) {
          ...
          (closure_node.nodedef != oren_nayar_diffuse_bsdf_id || name != "energy_compensation"))
      {
        ...
        return finish(false);
           ...
           closure_node.nodedef == generalized_schlick_bsdf_id)
  {
     ...
     * generalized_schlick_bsdf -> only the exact literal weight=0.0 subset.
     * Unsupported siblings in this exact91 batch (layer, conical/measured EDF)
     * remain absent from surface_closure_kind() and fail closed by name. */
    ...
    if (closure_node.nodedef == generalized_schlick_bsdf_id) {
      const pxr::UsdShadeInput weight = closure.GetInput(pxr::TfToken("weight"));
      if (!weight || weight.GetTypeName() != pxr::SdfValueTypeNames->Float ||
          weight.HasConnectedSource())
      {
        set_error(error_message,
                  string(generalized_schlick_bsdf_id) +
                      " requires literal weight=0.0 for the exact zero-contribution subset");
        return finish(false);
      }
      float weight_value = 0.0f;
      if (!weight.Get(&weight_value) || weight_value != 0.0f) {
        set_error(error_message,
                  string(generalized_schlick_bsdf_id) +
                      " only has a direct Cycles equivalent for literal weight=0.0");
        return finish(false);
      }
    }
    ...
    for (const pxr::UsdShadeInput &closure_input : closure.GetInputs()) {
      ...
      if (name != "weight" && name != "color" && name != "roughness" && name != "normal" &&
          ...
          name != "scatter_mode" &&
          (closure_node.nodedef != generalized_schlick_bsdf_id ||
           (name != "color0" && name != "color82" && name != "color90" &&
            name != "exponent")))
      {
