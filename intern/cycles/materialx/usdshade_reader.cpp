                       ...
                       string *error_message)
{
  ...
  if (nodedef == separate4_vector4_id) {
    ...
    if (input_value.HasConnectedSource()) {
      ...
      if (!input_value.Get(&value) || !color4_is_finite(value)) {
        set_error(error_message,
                  "ND_separate4_vector4 requires literal finite or connected vector4 input 'in'");
        return finish(false);
      }
  ...
  if (nodedef == separate3_vector3_id) {
    ...
    if (!read_vector3_operand(
            source, nodedef, "in", graph, &node, &active_vector3_shaders, depth + 1, error_message))
    {
      return finish(false);
    }
    node.outputs["outx"] = Type::Float;
  ...
  if (nodedef == separate4_color4_id) {
    ...
    if (input_value.HasConnectedSource()) {
      ...
      if (!input_value.Get(&value) || !color4_is_finite(value)) {
        set_error(error_message,
                  "ND_separate4_color4 requires literal finite or connected color4 input 'in'");
        return finish(false);
      }
      node.float4_inputs["in"] = make_float4(value[0], value[1], value[2], value[3]);
    }
